...
客户端和服务器之间的 HTTP 中间实体，Web 安全、应用集成、性能优化的重要组成模块。

### 缓存

Web 缓存（Web cache）或代理缓存（proxy cache）是一种特殊的 HTTP 代理服务器，可以将经过代理传送的常用文档复制保存下来，使常用页面副本保存在离客户端更近的地方

### 网关

网关（gateway）是连接其他应用程序的特殊 Web 服务器，作为其他服务器的中间实体使用，常用于将 HTTP 流量转换成其他的协议。

### 隧道

隧道（tunnel）是建立起来之后，就会在两条连接之间对原始数据进行盲转发的 HTTP 应用程序，常用来在一条或多条 HTTP 连接上转发非 HTTP 数据，转发时不会窥探数据。

一个常见用途是通过 HTTP 连接承载加密的安全套接字曾（SSL，Secure Sockets Layer）流量，这样 SSL 流量就可以穿过只允许 Web 流量通过的防火墙了。

![image](https://user-images.githubusercontent.com/37435717/85113999-cb847100-b24a-11ea-9336-ce4f72f9b636.png)

上图是一个 HTTP/SSL 隧道，它收到一条 HTTP 请求，要求建立一条到目的地址和端口的输出连接，实现在 HTTP 信道上通过隧道传输加密的 SSL 流量。

### Agent 代理

用户 Agent 代理（简称 Agent 代理）是代表用户发起 HTTP 请求的客户端程序。所有发布 Web 请求的应用程序都是 HTTP Agent 代理，如 Web 浏览器、爬虫
